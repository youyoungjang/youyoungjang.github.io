...
최종적으로 reconstructed data $\hat{\mathbf{x}_t}$ 는 $\mathcal{N}(\mathbf{\mu}_{\mathbf{x}_t}, \mathbf{\sigma}^2_{\mathbf{x}_t} \mathbf{I}) $ 에서 추출되고 $\mathbf{z}_t$ 로부터 생성됩니다.  

만약 $t$ 시점에서 $\hat{\mathbf{x}_t}$ 가 original data $\mathbf{x}_t$ 와 많이 다르다면 이를 이상치라고 판별할 수 있을 것입니다.  
